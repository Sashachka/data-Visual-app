//it doesn't get displayed in data vis as i decided to delete it but as it's original code from university(course) I leave it here.
    ...
    for(var j = 0; j < legend.length; j++)
    {
        ...
        for (var i = 0; i < this.series[legend[j]].length; i++) 
        {
          ...
          if (previous != null) {
           ...
           // working but it can be way better
            if (abs(mouseY - this.mapYToHeight(previous.percentage)) < 10 && mouseX > this.mapYearToWidth(previous.year) && mouseX < this.mapYearToWidth(current.year)){
              console.log("we are in abs function to make line bold" +"x: "+  mouseX+ "y: " + mouseY);
              strokeWeight(10);
              } else {
                strokeWeight(1);
              }
